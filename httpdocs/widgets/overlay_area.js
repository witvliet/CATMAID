...
)
{
  ...
  this.createMidpoints = function() {    
    ...
    if (l > 1) //if "L" > 1
    {
      ...
      for (i = 0; i < l - 1; i++)
      {
        ...
        midx.push(mx);
        midy.push(my);
        mdots.push(new VertexDot(mx, my, this.r / 2, this.rcatch / 2, i, this, true));
      }
      mx = (this.x[l-1] + this.x[0]) / 2;
      my = (this.y[l-1] + this.y[0]) / 2;
      midx.push(mx);
      midy.push(my);
      mdots.push(new VertexDot(mx, my, this.r / 2, this.rcatch / 2, l-1, this, true));
    }
  ...
  // Set the mode to "createvertices", or "editvertices"
  this.setMode = function(mode) {
    if (mode === "createvertices" || mode === "editvertices"){
      this.mode = mode;
    }      
  };
  
  this.switchMode = function() {
    if (this.mode === "createvertices") {
      this.mode = "editvertices";
