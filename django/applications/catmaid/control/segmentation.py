...
def retrieve_components_for_location(project_id, stack_id, x, y, z, limit=10):
    ...
    with closing(h5py.File(fpath, 'r')) as hfile:
        ...
        print("extract components ....", file=sys.stderr)
        start = time.time()
        ...
        print("took", delta, file=sys.stderr)

        print("create components ....", file=sys.stderr)
        start = time.time()
    ...
    with closing(h5py.File(filename, 'w')) as hfile:
        ...
        for z in whitelist:
            ...
            for freeDrawing in all_free_drawings:
                ...
                print('freedrawing type', freeDrawing.type, typename, file=sys.stderr)
...
def get_tile(request, project_id=None, stack_id=None):
    ...
    with closing(h5py.File(fpath, 'r')) as hfile:
        ...
        #print('hdf5 path', hdfpath, image_data, data, data.shape, file=sys.stderr)
